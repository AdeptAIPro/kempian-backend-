...
from app.models import db, User, CandidateProfile, CandidateSkill, CandidateEducation, CandidateExperience, CandidateCertification, UserSocialLinks, UserKPIs, UserSkillGap, UserLearningPath, UserAchievement, UserGoal, UserSchedule, LearningModule, LearningCourse, JDSearchLog, UserTrial, CeipalIntegration, UserImage
...
def delete_profile():
    ...
    try:
        ...
        # Remove learning modules and courses
        ...
        for path in learning_paths:
            LearningCourse.query.filter(
                LearningCourse.module_id.in_(
                    db.session.query(LearningModule.id).filter_by(learning_path_id=path.id)
                )
