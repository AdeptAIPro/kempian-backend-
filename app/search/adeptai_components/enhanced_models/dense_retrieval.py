...
def create_production_matcher_from_dynamodb(table, index_path: str = "indexes/resume_index"):
    ...
    if matcher.dense_retriever.index is None:
        logger.info("Building new index from DynamoDB (limited to 300 candidates)...")
        
        # Scan DynamoDB table (limited to 300 candidates)
        all_items = []
        max_candidates = 300
        ...
        while len(all_items) < max_candidates:
            remaining = max_candidates - len(all_items)
            if remaining < 100:
            ...
            if len(all_items) >= max_candidates:
                logger.info(f"✅ Reached limit of {max_candidates} candidates")
                break
