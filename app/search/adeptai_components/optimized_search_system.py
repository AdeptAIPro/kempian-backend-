...
# Add parent directory to path to find Sagemaker folder
...
if _dependencies_available:
    ...
    def _set_env_if_present(env_key: str, value: Optional[str]) -> None:
        if value is not None:
            os.environ[env_key] = value

    _set_env_if_present('ANTHROPIC_API_KEY', os.getenv('ANTHROPIC_API_KEY'))
    _set_env_if_present('AWS_ACCESS_KEY_ID', os.getenv('AWS_ACCESS_KEY_ID'))
    _set_env_if_present('AWS_SECRET_ACCESS_KEY', os.getenv('AWS_SECRET_ACCESS_KEY'))
    os.environ.setdefault('AWS_DEFAULT_REGION', os.getenv('AWS_REGION', 'ap-south-1'))

# Database configuration
DATABASE_TABLE_NAME = os.getenv('DYNAMODB_TABLE_NAME', 'resume_metadata')
MAX_DYNAMODB_CANDIDATES = None
...
MOCK_CANDIDATES = [
    {
        ...
        'designations_with_experience': ['Physician - 8 years'],
        'domain_tag': 'healthcare',
        'certifications': ['Board Certified Internal Medicine'],
        'current_position': 'Attending Physician',
    ...
    },
    {
        ...
        'designations_with_experience': ['Registered Nurse - 7 years'],
        'domain_tag': 'healthcare',
        'certifications': ['ACLS', 'BLS'],
        'current_position': 'ICU Nurse',
        'location': 'Chicago, IL'
    },
    {
        'filename': '',
        ...
        'designations_with_experience': ['Cardiologist - 12 years'],
        'domain_tag': 'healthcare',
        'certifications': ['FACC'],
    ...
    },
    {
        ...
        'education': ['BS Nursing'],
        'designations_with_experience': ['Pediatric Nurse - 5 years'],
        'domain_tag': 'healthcare',
        'certifications': ['PALS'],
        'current_position': 'Pediatric RN',
        'location': 'Seattle, WA'
    },
    {
        'filename': '',
        ...
        'designations_with_experience': ['Emergency Physician - 10 years'],
        'domain_tag': 'healthcare',
        'certifications': ['ATLS'],
        'current_position': 'ER Physician',
    ...
    },
    {
        ...
        'education': ['BS Nursing'],
        'designations_with_experience': ['Surgical Nurse - 6 years'],
        'domain_tag': 'healthcare',
        'certifications': ['CNOR'],
        'current_position': 'OR Nurse',
        'location': 'Houston, TX'
    }
]
...
TECH_SKILL_KEYWORDS = {
    ...
    'frameworks': ['react', 'vue', 'angular', 'node.js', 'nodejs', 'django', 'flask', 'fastapi',
                   'spring', 'express', 'laravel', 'rails', '.net', 'asp.net', 'next.js', 'nuxt', 'svelte'],
    'cloud': ['aws', 'amazon web services', 'azure', 'gcp', 'google cloud', 'docker', 'kubernetes', 'k8s',
              'terraform', 'ansible', 'jenkins', 'github actions', 'gitlab ci', 'cloudformation', 'ec2', 's3'],
    'database': ['sql', 'mysql', 'postgresql', 'postgres', 'mongodb', 'redis', 'cassandra',
                 'elasticsearch', 'dynamodb', 'oracle', 'nosql', 'sqlite', 'mariadb', 'snowflake', 'bigquery'],
    'ml_ai': ['tensorflow', 'pytorch', 'keras', 'scikit-learn', 'sklearn', 'pandas', 'numpy',
              'machine learning', 'ml', 'deep learning', 'ai', 'artificial intelligence',
              'neural networks', 'nlp', 'natural language processing', 'computer vision', 'cv', 'llm', 'xgboost'],
    'data': ['data analysis', 'data science', 'big data', 'spark', 'hadoop', 'kafka', 'tableau',
             'power bi', 'matplotlib', 'seaborn', 'jupyter', 'notebook', 'data visualization',
             'statistics', 'statistical analysis', 'apache beam'],
    'devops': ['docker', 'kubernetes', 'ci/cd', 'jenkins', 'terraform', 'ansible', 'chef',
               'puppet', 'git', 'github', 'gitlab', 'linux', 'unix', 'bash', 'shell scripting', 'prometheus', 'grafana'],
    'frontend': ['react', 'vue', 'angular', 'javascript', 'typescript', 'html', 'css',
                 'sass', 'less', 'redux', 'webpack', 'next.js', 'nuxt', 'vue.js', 'tailwind', 'storybook'],
    'backend': ['node.js', 'python', 'java', 'spring', 'django', 'flask', 'express', 'api',
                'rest', 'graphql', 'microservices', 'fastapi', 'nest.js', 'asp.net core'],
    'security': ['security', 'penetration testing', 'vulnerability', 'siem', 'splunk', 'soc', 'threat detection'],
    'mobile': ['android', 'ios', 'swift', 'kotlin', 'react native', 'flutter', 'xcode'],
}
...
def _normalize_list_field(value: Any) -> List[str]:
    ...
    if isinstance(value, str):
        parts = [part.strip() for part in LIST_SPLIT_PATTERN.split(value) if part.strip()]
        return parts
...
def _normalize_experience(value: Any) -> float:
    if value is None:
        return 0.0
    try:
        return float(value)
...
def _normalize_candidate_item(item: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize raw DynamoDB candidate record into canonical structure."""
    normalized = dict(item or {})
    
    normalized['email'] = _normalize_string(
        normalized.get('email') or normalized.get('Email') or normalized.get('EMAIL')
    )
    normalized['full_name'] = _normalize_string(
        normalized.get('full_name') or normalized.get('name') or normalized.get('FullName')
    )
    normalized['phone'] = _normalize_string(normalized.get('phone') or normalized.get('mobile'))
    normalized['filename'] = _normalize_string(
        normalized.get('filename') or normalized.get('file_name') or normalized.get('resume_filename')
    )
    normalized['resume_text'] = _normalize_string(
        normalized.get('resume_text') or normalized.get('resume') or normalized.get('resume_summary')
    )
    normalized['skills'] = _normalize_list_field(normalized.get('skills'))
    normalized['education'] = _normalize_list_field(normalized.get('education'))
    designations_raw = normalized.get('designations_with_experience') or normalized.get('designations')
    if isinstance(designations_raw, dict):
        normalized['designations_with_experience'] = [str(k).strip() for k in designations_raw.keys() if str(k).strip()]
    else:
        normalized['designations_with_experience'] = _normalize_list_field(designations_raw)

    certifications_raw = normalized.get('certifications')
    if isinstance(certifications_raw, dict):
        normalized['certifications'] = [str(k).strip() for k in certifications_raw.keys() if str(k).strip()]
    else:
        normalized['certifications'] = _normalize_list_field(certifications_raw)
    normalized['domain_tag'] = _normalize_string(normalized.get('domain_tag'))
    normalized['current_position'] = _normalize_string(
        normalized.get('current_position') or normalized.get('current_role')
    )
    normalized['location'] = _normalize_string(
        normalized.get('location') or normalized.get('city') or normalized.get('loc')
    )
    normalized['sourceURL'] = _normalize_string(
        normalized.get('sourceURL') or normalized.get('source_url') or normalized.get('sourceUrl')
    )
    normalized['total_experience_years'] = _normalize_experience(
        normalized.get('total_experience_years') or normalized.get('experience_years')
    )
    
    # Ensure optional fields exist even if empty
    normalized.setdefault('education', [])
    normalized.setdefault('designations_with_experience', [])
    normalized.setdefault('certifications', [])
    normalized.setdefault('domain_tag', 'general')
    normalized.setdefault('current_position', '')
    normalized.setdefault('location', '')
    normalized.setdefault('sourceURL', '')
    normalized.setdefault('filename', '')
    
    return normalized

def _flatten_tech_keywords() -> Set[str]:
    flattened = set()
    for keywords in TECH_SKILL_KEYWORDS.values():
        flattened.update(k.lower() for k in keywords)
    return flattened

TECH_KEYWORD_SET = _flatten_tech_keywords()
...
def get_candidates_with_fallback():
    ...
    """
    # Try ALL cache first
    max_candidates = MAX_DYNAMODB_CANDIDATES or float('inf')

    if _is_cache_valid(CACHE_FILE_ALL):
        print("🔄 Loading ALL candidates from cache...")
        cached_all = _load_from_cache(CACHE_FILE_ALL)
        if cached_all:
            cached_all = [_normalize_candidate_item(item) for item in cached_all]
    ...
    try:
        ...
        if _dependencies_available and 'boto3' in globals():
            ...
            table_name = os.getenv('DYNAMODB_TABLE_NAME', DATABASE_TABLE_NAME)
            ...
            if aws_access_key and aws_secret_key:
                ...
                max_candidates = MAX_DYNAMODB_CANDIDATES
                if max_candidates:
                    print(f"📊 Scanning DynamoDB table: {table_name} (loading up to {max_candidates} candidates)")
                else:
                    print(f"📊 Scanning DynamoDB table: {table_name} (loading all candidates)")
                all_items = []
                scan_kwargs = {
                    'ProjectionExpression': 'email, full_name, skills, resume_text, total_experience_years, phone, sourceURL, filename, education, designations_with_experience, domain_tag, certifications, current_position, #loc',
                    'ExpressionAttributeNames': {'#loc': 'location'}
                }
                if max_candidates:
                    scan_kwargs['Limit'] = max(1, min(100, max_candidates))
                
                start_time = time.time()
                batch_count = 0
                
                while True:
                    remaining_slots = max_candidates - len(all_items) if max_candidates else None
                    if remaining_slots is not None and remaining_slots <= 0:
                        break
                    if remaining_slots is not None and 'Limit' in scan_kwargs:
                        scan_kwargs['Limit'] = max(1, min(scan_kwargs['Limit'], remaining_slots))
                    ...
                    if remaining_slots is not None and len(items) > remaining_slots:
                    ...
                    if max_candidates and len(all_items) >= max_candidates:
                        break
                ...
                if all_items:
                    normalized_items = [_normalize_candidate_item(item) for item in all_items]
                    print(f"✅ Successfully loaded {len(normalized_items)} candidates from DynamoDB in {load_time:.2f}s")
                    # Save caches
...
class OptimizedDomainClassifier:
    ...
    def __init__(self):
        ...
        try:
            ...
            if use_sagemaker and unified_endpoint:
                from Sagemaker.sagemaker_client import SageMakerDomainClassifierClient  # type: ignore
                self.ml_classifier = SageMakerDomainClassifierClient(endpoint_name=None, region=region)
                # Provide a consistent interface
                self.ml_classifier.is_trained = True  # type: ignore[attr-defined]
                self.use_ml = True
                print("✅ Using SageMaker Unified Endpoint for Domain Classification")
        except Exception as e:
...
class OptimizedSearchSystem:
    ...
    def __init__(self, use_mock_data: bool = False, enable_custom_llm: bool = True,
                 behavior_pipeline: Optional[Any] = None,
                 bias_monitor: Optional[Any] = None,
                 market_intelligence: Optional[Any] = None,
                 skill_taxonomy: Optional[Any] = None,
                 context_weighting: Optional[Any] = None,
                 explainable_ai: Optional[Any] = None):
        ...
        self.use_custom_llm = enable_custom_llm and CUSTOM_LLM_AVAILABLE
        ...
        try:
            ...
            _unified_ep = os.getenv('SAGEMAKER_UNIFIED_ENDPOINT', 'unified-ml-pipeline-endpoint')
            _region = os.getenv('SAGEMAKER_REGION', os.getenv('AWS_REGION', 'ap-south-1'))
            if _use_sm and _unified_ep:
                from Sagemaker.sagemaker_client import SageMakerLTRClient  # type: ignore
                class _LTRAdapter:
                    def __init__(self, endpoint: Optional[str], region: str):
                        # Always pass None to use unified endpoint from env
                        self.client = SageMakerLTRClient(None, region)
                    ...
                    def predict(self, query, candidates, feature_scores=None):
                        ...
                        # Fallback
                        return [0.0] * len(candidates)
                # Use unified endpoint - pass None as endpoint name
                self.ltr_model = _LTRAdapter(None, _region)
                print("✅ LTR via SageMaker initialized")
        except Exception as e:
            print(f"⚠️ Failed to initialize SageMaker LTR: {e}")
            self.ltr_model = None
            ...
            if _use_sm and _unified_ep:
                ...
                class _EnhancerAdapter:
                    def __init__(self, endpoint: str, region: str):
                        self.client = SageMakerLLMQueryEnhancerClient(None, region)
                ...
                self.llm_query_enhancer = _EnhancerAdapter(_unified_ep, _region)
        ...
        self.skill_taxonomy = skill_taxonomy or SKILL_TAXONOMY_AVAILABLE and get_skill_taxonomy and get_skill_taxonomy()
        self.use_skill_taxonomy = bool(self.skill_taxonomy)
        if self.skill_taxonomy:
            print("✅ Skill Taxonomy initialized")
        
        # Initialize Context-Aware Weighting
        self.context_weighting = context_weighting or CONTEXT_WEIGHTING_AVAILABLE and get_context_weighting and get_context_weighting()
        self.use_context_weighting = bool(self.context_weighting)
        if self.context_weighting:
            print("✅ Context-Aware Weighting initialized")
        
        # Initialize Behavioural Analysis Pipeline
        self.behavioral_pipeline = behavior_pipeline or BEHAVIOURAL_ANALYSIS_AVAILABLE and get_pipeline and get_pipeline('lightweight')
        self.use_behavioral_analysis = bool(self.behavioral_pipeline)
        if self.behavioral_pipeline:
            print(f"✅ Behavioural Analysis Pipeline initialized ({self.behavioral_pipeline.config_name})")
        ...
        self.bias_monitor = bias_monitor or BIAS_PREVENTION_AVAILABLE and BiasMonitor and BiasMonitor()
        self.use_bias_prevention = bool(self.bias_monitor)
        ...
        self.explainable_ai = explainable_ai or EXPLAINABLE_AI_AVAILABLE and ExplainableRecruitmentAI and ExplainableRecruitmentAI()
        self.use_explainable_ai = bool(self.explainable_ai)
        if self.explainable_ai:
            print("✅ Explainable AI initialized")
        
        # Initialize Market Intelligence
        self.market_intelligence = market_intelligence or MARKET_INTELLIGENCE_AVAILABLE and MarketIntelligenceAPI and MarketIntelligenceAPI()
        self.skills_forecaster = self.market_intelligence and SkillsForecaster and self.market_intelligence.skills_forecaster
        self.compensation_benchmarker = self.market_intelligence and CompensationBenchmarker and self.market_intelligence.compensation_benchmarker
        self.use_market_intelligence = bool(self.market_intelligence)
        if self.market_intelligence:
            print("✅ Market Intelligence API initialized")
            if self.skills_forecaster:
                print("✅ Skills Forecaster initialized")
            if self.compensation_benchmarker:
        ...
        try:
            ...
            if _use_sm and _unified_ep:
                ...
                class _DenseAdapter:
                    def __init__(self, endpoint: str, region: str):
                        self.client = SageMakerDenseRetrievalClient(None, region)
                    def find_matches(self, query: str, top_k: int = 20):
                        return self.client.search(query, top_k=top_k)
                self.production_matcher = _DenseAdapter(_unified_ep, _region)
        ...
        try:
            ...
            if _use_sm and _unified_ep:
                ...
                class _EmbeddingServiceAdapter:
                    def __init__(self, endpoint: str, region: str):
                        self.client = SageMakerEmbeddingClient(None, region)
                    def get_embedding(self, text: str, model_type: str = 'general'):
                        import numpy as _np
                        try:
                            emb = self.client.encode(text, model_type)
                ...
                self.embedding_service = _EmbeddingServiceAdapter(_unified_ep, _region)
    ...
    def _aggregate_candidate_skills(self, record: Dict[str, Any]) -> List[str]:
        """Aggregate candidate skills from multiple metadata fields and resume text."""
        aggregated: List[str] = []
        ...
        _add_values(record.get('skills', []))
        _add_values(record.get('designations_with_experience'))
        _add_values(record.get('certifications'))
        _add_values(record.get('current_position'))
        _add_values(record.get('education'))
        _add_values(record.get('domain_tag'))

        # Extract technical keywords from resume text
        resume_text = str(record.get('resume_text', '') or '').lower()
        if resume_text:
            for keyword in TECH_KEYWORD_SET:
                if keyword and keyword in resume_text:
                    aggregated.append(keyword)

        # Also include skills from source URL filename hints
        source_url = str(record.get('sourceURL', '') or '').lower()
        if source_url:
            for keyword in TECH_KEYWORD_SET:
                if keyword and keyword in source_url:
                    aggregated.append(keyword)
    ...
    def _infer_query_domain(self, query: str) -> Tuple[str, float]:
        if not query:
            return "unknown", 0.0
        query_lower = query.lower()
        tech_keywords = {"developer", "engineer", "python", "data", "devops", "frontend",
                         "backend", "full stack", "software", "cloud", "ai", "ml", "react",
                         "java", "javascript", "kubernetes", "aws", "product", "qa", "sre"}
        healthcare_keywords = {"nurse", "physician", "medical", "health", "clinic", "patient",
                               "clinical", "pharmacy", "dentist", "therapy", "therapist",
                               "radiology", "cardio", "surgery", "hospital", "rn", "lpn",
                               "paramedic", "care"}
        tech_hits = sum(1 for word in tech_keywords if word in query_lower)
        healthcare_hits = sum(1 for word in healthcare_keywords if word in query_lower)
        if tech_hits and tech_hits >= healthcare_hits * 2:
            confidence = min(0.4 + 0.15 * tech_hits, 0.95)
            return "it", confidence
        if healthcare_hits and healthcare_hits >= tech_hits * 2:
            confidence = min(0.4 + 0.15 * healthcare_hits, 0.95)
            return "healthcare", confidence
        return "unknown", 0.0

    def _determine_candidate_domain(self, candidate: Dict[str, Any]) -> Tuple[str, float]:
        domain_tag = candidate.get('domain_tag') or candidate.get('domain')
        if isinstance(domain_tag, str) and domain_tag:
            return domain_tag.lower(), 0.9
        # heuristic from skills
        skills_text = ' '.join(candidate.get('skills') or [])
        skills_text += ' ' + (candidate.get('resume_text') or '')
        skills_text_lower = skills_text.lower()
        tech_indicators = ["python", "java", "javascript", "software", "developer", "engineer",
                           "kubernetes", "aws", "cloud", "data", "devops", "sql", "react",
                           "django", "flask", "pipeline", "api", "ci/cd", "terraform",
                           "docker", "linux"]
        healthcare_indicators = ["patient", "care", "nurse", "medical", "hospital", "clinic",
                                 "therapy", "healthcare", "rn", "bsn", "pharmacy", "charting",
                                 "surgery", "diagnosis", "therapist", "physician", "emt", "icu",
                                 "lpn", "acls", "cna", "triage"]
        tech_hits = sum(1 for kw in tech_indicators if kw in skills_text_lower)
        healthcare_hits = sum(1 for kw in healthcare_indicators if kw in skills_text_lower)
        if tech_hits and tech_hits >= healthcare_hits * 2:
            return "it", min(0.3 + 0.1 * tech_hits, 0.85)
        if healthcare_hits and healthcare_hits >= tech_hits * 2:
            return "healthcare", min(0.3 + 0.1 * healthcare_hits, 0.85)
        return "unknown", 0.0
    ...
    def _init_custom_llm_components(self):
        ...
        if self.use_custom_llm:
            ...
            sm_unified_ep = os.getenv('SAGEMAKER_UNIFIED_ENDPOINT', 'unified-ml-pipeline-endpoint')
            sm_region = os.getenv('SAGEMAKER_REGION', os.getenv('AWS_REGION', 'ap-south-1'))
            self._sagemaker_embeddings = False
            if use_sm and sm_unified_ep:
                try:
                    from Sagemaker.sagemaker_client import SageMakerEmbeddingClient  # type: ignore
                    class _EmbeddingAdapter:
                        ...
                        def build_index(self, *_args, **_kwargs):
                            self.is_index_ready = True
                    self.custom_embedding_model = _EmbeddingAdapter(sm_unified_ep, sm_region)
            ...
            if not getattr(self.custom_embedding_model, 'skip_training', False):
                ...
                if candidate_texts:
                    can_fit = hasattr(self.custom_embedding_model, 'fit')
                    can_build_index = hasattr(self.custom_embedding_model, 'build_index')
                    can_batch_encode = hasattr(self.custom_embedding_model, 'batch_encode')
                    if can_fit and can_build_index and can_batch_encode:
                        ...
                        print("ℹ️ Embedding model does not support training/building an index; skipping training step")
                        self.use_custom_llm = can_batch_encode
                else:
    ...
    def search(self, query: str, top_k: int = 10, **kwargs):
        ...
        if self.use_optimized_cache and self.optimized_cache and use_optimized_cache:
            try:
                ...
                cached_results = self.optimized_cache.search(query, limit=top_k * 2)  # Get more candidates for scoring
                if cached_results:
                    ...
                    for result in cached_results:
                        if isinstance(result, dict):
                            ...
                            candidate_data = {
                                ...
                                'total_experience_years': result.get('total_experience_years', original_candidate.get('total_experience_years', 0) if original_candidate else 0),
                                'phone': result.get('phone', original_candidate.get('phone', '') if original_candidate else ''),
                                'sourceURL': result.get('sourceURL', original_candidate.get('sourceURL', '') if original_candidate else ''),
                                ...
                                'location': result.get('location', original_candidate.get('location', '') if original_candidate else ''),
                                'education': result.get('education', original_candidate.get('education', []) if original_candidate else []),
                                'designations_with_experience': result.get('designations_with_experience', original_candidate.get('designations_with_experience', []) if original_candidate else []),
                                'domain_tag': result.get('domain_tag', original_candidate.get('domain_tag', '') if original_candidate else ''),
                                'certifications': result.get('certifications', original_candidate.get('certifications', []) if original_candidate else []),
                                'current_position': result.get('current_position', original_candidate.get('current_position', '') if original_candidate else ''),
                                'filename': result.get('filename', original_candidate.get('filename', '') if original_candidate else ''),
                                'seniority_level': result.get('seniority_level', original_candidate.get('seniority_level', 'mid') if original_candidate else 'mid')
                            }

                            candidate_data['skills'] = _normalize_list_field(candidate_data.get('skills'))
                            candidate_data['education'] = _normalize_list_field(candidate_data.get('education'))
                            candidate_data['designations_with_experience'] = _normalize_list_field(candidate_data.get('designations_with_experience'))
                            candidate_data['certifications'] = _normalize_list_field(candidate_data.get('certifications'))
                            candidate_data['domain_tag'] = _normalize_string(candidate_data.get('domain_tag'))
                            candidate_data['current_position'] = _normalize_string(candidate_data.get('current_position'))
                            candidate_data['location'] = _normalize_string(candidate_data.get('location'))
                            candidate_data['filename'] = _normalize_string(candidate_data.get('filename'))
                            candidate_data['sourceURL'] = _normalize_string(candidate_data.get('sourceURL'))
        ...
        if self.use_explainable_ai and self.explainable_ai:
            ...
            for idx, result in enumerate(results):
                try:
                    ...
                    education_profile_value = result.get('education', '')
                    if isinstance(education_profile_value, list):
                        education_profile_value = ', '.join(education_profile_value)
                    candidate_profile = {
                        'full_name': result.get('full_name', ''),
                        'email': result.get('email', ''),
                        ...
                        'education': education_profile_value,
        ...
        required_skills_for_injection = self._extract_required_skills_from_query(query)
        results = self._inject_additional_skill_matches(query, results, required_skills_for_injection, top_k)
        
        # Apply domain filtering if enabled
        enable_domain_filtering = kwargs.get('enable_domain_filtering', True)  # Default to True
        results_before_domain = list(results)
        if enable_domain_filtering and results:
            ...
            if filtered_results:
                ...
                print("⚠️ Domain filtering removed all candidates; reverting to pre-filter results")
                results = results_before_domain
        ...
        # Sort by: 1) Has skill match (boolean - CRITICAL), 2) Skill match score, 3) Final score
        ...
        results.sort(key=lambda x: (
            ...
            x.get('final_score', 0.0)  # Tertiary: Final score (higher is better)
        ), reverse=True)
        ...
        # Sort each group
        with_skills.sort(key=lambda x: (x.get('skill_match_score', 0.0), x.get('final_score', 0.0)), reverse=True)
    ...
    def _hybrid_search(self, query: str, enhanced_query: Dict[str, Any], top_k: int) -> List[Dict[str, Any]]:
        ...
        for processed_candidate in self._processed_candidates:
            ...
            if final_score > 0.1:  # Only include candidates with some relevance
                ...
                result = {
                    ...
                    'designations_with_experience': candidate.get('designations_with_experience', []),
                    ...
                    'domain': candidate.get('domain_tag', '') or 'general',
                    'ai_explanation': f"Hybrid match: keyword={keyword_score:.2f}, semantic={semantic_score:.2f}"
                }
                result['skills'] = _normalize_list_field(result.get('skills'))
                result['education'] = _normalize_list_field(result.get('education'))
                result['designations_with_experience'] = _normalize_list_field(result.get('designations_with_experience'))
                result['certifications'] = _normalize_list_field(result.get('certifications'))
                result['domain_tag'] = _normalize_string(result.get('domain_tag'))
                result['current_position'] = _normalize_string(result.get('current_position'))
                result['location'] = _normalize_string(result.get('location'))
                result['filename'] = _normalize_string(result.get('filename'))
                result['sourceURL'] = _normalize_string(result.get('sourceURL'))
        ...
        if self.use_custom_llm and self.custom_embedding_model and self.custom_embedding_model.is_index_ready():
            ...
            for i, score in zip(idxs, scores):
                ...
                results[-1]['skills'] = _normalize_list_field(results[-1].get('skills'))
                results[-1]['education'] = _normalize_list_field(results[-1].get('education'))
                results[-1]['designations_with_experience'] = _normalize_list_field(results[-1].get('designations_with_experience'))
                results[-1]['certifications'] = _normalize_list_field(results[-1].get('certifications'))
                results[-1]['domain_tag'] = _normalize_string(results[-1].get('domain_tag'))
                results[-1]['current_position'] = _normalize_string(results[-1].get('current_position'))
                results[-1]['location'] = _normalize_string(results[-1].get('location'))
                results[-1]['filename'] = _normalize_string(results[-1].get('filename'))
                results[-1]['sourceURL'] = _normalize_string(results[-1].get('sourceURL'))
    ...
    def _apply_job_fit_scoring(self, query: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for result in results:
            try:
                ...
                education_value = result.get('education', '')
                if isinstance(education_value, list):
                    education_value = ', '.join(education_value)
                candidate = {
                    'full_name': result.get('full_name', ''),
                    'email': result.get('email', ''),
                    'skills': result.get('skills', []),
    ...
    def _normalize_skill(self, skill: str) -> str:
        """Normalize skill name for matching - IMPROVED with more aliases"""
        if not skill:
            return ""
        
        skill_lower = skill.lower().strip()
        
        # IMPROVED: Expanded normalization map with more skill aliases
        skill_aliases = {
        ...
        # Check if skill contains any alias key
    ...
    def _calculate_skill_match_score(
        ...
        """
        Calculate skill match score with skill taxonomy and context-aware weighting.
        ...
        if self.use_skill_taxonomy and self.skill_taxonomy:
            try:
                ...
                taxonomy_score = match_result['overall_score']
                taxonomy_details = {
        ...
        partial_score = len(partial_matches) / len(required_skills_normalized) * 0.75 if required_skills_normalized else 0
        ...
        if match_rate >= 0.2 and total_score < 0.2:
            total_score = 0.2
        
        basic_score = min(1.0, total_score)
        basic_details = {
            "matches": matches,
        ...
        # Combine taxonomy and fallback results
        if taxonomy_score is not None and taxonomy_details is not None:
            combined_details = dict(taxonomy_details)
            combined_details['taxonomy_score'] = taxonomy_score
            combined_details['fallback_score'] = basic_score
            combined_details['basic_match_rate'] = basic_details['match_rate']
            combined_details['fallback_matches'] = basic_details['matches']

            # Merge match lists
            combined_matches = sorted(set(combined_details.get('matches', [])) | set(basic_details['matches']))
            combined_exact = sorted(set(combined_details.get('exact_matches', [])) | set(basic_details['exact_matches']))
            combined_partial = sorted(set(combined_details.get('partial_matches', [])) | set(basic_details['partial_matches']))

            combined_details['matches'] = combined_matches
            combined_details['exact_matches'] = combined_exact
            combined_details['partial_matches'] = combined_partial
            combined_details['total_matches'] = len(combined_matches)
            combined_details['match_rate'] = max(combined_details.get('match_rate', 0.0), basic_details['match_rate'])
            combined_details['total_required'] = max(combined_details.get('total_required', 0), basic_details['total_required'])

            # Choose the stronger score
            combined_score = max(taxonomy_score, basic_score)
            if basic_score > taxonomy_score:
                combined_details['method'] = 'taxonomy+fallback'
            else:
                combined_details.setdefault('method', 'taxonomy')

            return min(1.0, combined_score), combined_details

        # No taxonomy score available, use basic fallback
        return basic_score, basic_details
    
    def _apply_skill_based_scoring(self, query: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Apply skill-based scoring to improve accuracy"""
        if not results:
            return results
        ...
        try:
            ...
            for result in results:
                ...
                # PRIORITIZE SKILL MATCHING - candidates with skills ALWAYS rank above those without
                original_score = result.get('final_score', 0.0)
                
                if skill_score > 0:
                    # Coverage ratio of matched required skills
                    coverage_ratio = match_details.get('total_matches', 0) / max(1, match_details.get('total_required', 1))
                    # Strengthen signal: combine taxonomy score with coverage to boost well-aligned candidates
                    skill_signal = min(1.0, skill_score + 0.35 * coverage_ratio)
                    boosted_score = min(1.0, 0.6 * skill_signal + 0.4 * original_score)
                    if boosted_score < skill_signal:
                        boosted_score = skill_signal
                    result['final_score'] = max(original_score, boosted_score)
                    ...
                    penalty_score = min(original_score * 0.1, 0.03)
                    result['final_score'] = penalty_score
                    result['no_skill_match_penalty'] = True
                    result['has_skill_match'] = False
                
                scored_results.append(result)
            
            # MAXIMUM PRIORITY RERANKING: Skill matches first, then by skill match score, then final score
            # Sort candidates with skill matches first, then by skill match score (highest first), then final score
            ...
            # Sort candidates WITH skills by: skill_match_score (primary), then final_score (secondary)
            candidates_with_skills.sort(key=lambda x: (
                x.get('skill_match_score', 0.0),  # Primary: skill match score
                x.get('final_score', 0.0)  # Secondary: final score
            ), reverse=True)
    ...
    def _find_additional_candidates_by_skills(
        self,
        query: str,
        required_skills: List[str],
        ...
        """Search the full candidate pool for additional skill matches to supplement results."""
        if limit <= 0 or not required_skills:
            return []
        
        additional_results: List[Dict[str, Any]] = []
        processed_candidates = getattr(self, '_processed_candidates', [])
        
        for processed_candidate in processed_candidates:
            candidate = processed_candidate.get('original')
            if not candidate:
                continue
            email = candidate.get('email')
            ...
            candidate_skills = self._aggregate_candidate_skills(candidate)
            if not candidate_skills:
                continue
            
            candidate_context = candidate.get('resume_text', '') or ' '.join(candidate_skills)
            score, details = self._calculate_skill_match_score(
                candidate_skills,
                required_skills,
                query=query,
                candidate_context=candidate_context
            ...
            result = {
                ...
                'domain_tag': candidate.get('domain_tag', ''),
                'sourceURL': candidate.get('sourceURL', ''),
                'database_index': candidate.get('database_index', 0),
                'skill_match_score': score,
                'skill_match_details': details,
                'matched_skills': details['matches'],
                'required_skills': required_skills,
                'has_skill_match': True,
                'source': 'skill_match_fallback',
            }
            result['final_score'] = min(1.0, 0.65 * score + 0.25)
            result['similarity_score'] = score
            result['skill_boost'] = result['final_score']
            additional_results.append(result)
        
        additional_results.sort(key=lambda r: (r.get('skill_match_score', 0.0), r.get('final_score', 0.0)), reverse=True)
        return additional_results[:limit]

    def _inject_additional_skill_matches(
        self,
        query: str,
        ...
        """Ensure enough candidates with meaningful skill matches appear in the final set."""
        if not required_skills or not results:
            return results
        
        min_skill_score = 0.05 if len(required_skills) >= 3 else 0.03
        strong_matches = [r for r in results if r.get('skill_match_score', 0.0) >= min_skill_score]
        weak_matches = [r for r in results if 0 < r.get('skill_match_score', 0.0) < min_skill_score]
        no_matches = [r for r in results if r.get('skill_match_score', 0.0) == 0]
        
        needed = max(0, top_k - len(strong_matches))
        if needed > 0:
            exclude_emails = {r.get('email') for r in results}
            additional = self._find_additional_candidates_by_skills(
                query=query,
                required_skills=required_skills,
                exclude_emails=exclude_emails,
                limit=needed * 3
            )
            additional = [r for r in additional if r.get('skill_match_score', 0.0) >= min_skill_score]
            if additional:
                strong_matches.extend(additional[:max(0, top_k - len(strong_matches))])
        
        combined = strong_matches + weak_matches + no_matches
    ...
    def _apply_domain_filtering(self, query: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            heuristic_query_domain, heuristic_query_conf = self._infer_query_domain(query)
            query_domain = heuristic_query_domain
            query_confidence = heuristic_query_conf
            ...
            if self.use_behavioral_analysis and self.behavioral_pipeline:
                if hasattr(self.behavioral_pipeline, 'domain_bert'):
                    ...
                    try:
                        ...
                        domain_bert_value = getattr(domain_bert_result, 'value', None)
                        if domain_bert_value:
                            query_domain = domain_bert_value
                            query_confidence = max(query_confidence, 0.85)
                            print(f"🔍 Query domain (BERT): {query_domain}")
                    except Exception as e:
                        print(f"⚠️ Domain BERT detection failed: {e}, using heuristic/classifier")
            
            if query_domain in ('unknown', ''):
                print("⚠️ Query domain unknown; skipping strict domain filtering")
            ...
            filtered_results = []
            for result in results:
                candidate_domain, candidate_confidence = self._determine_candidate_domain(result)
                if candidate_domain in ('unknown', '') and hasattr(self.domain_classifier, 'classify_candidate'):
                    try:
                        candidate_domain, candidate_confidence = self.domain_classifier.classify_candidate(result)
                    except Exception:
                        candidate_domain, candidate_confidence = 'unknown', 0.0
                elif candidate_domain in ('unknown', ''):
                    candidate_text = result.get('resume_text', '') or ' '.join(result.get('skills', []))
                    candidate_domain, candidate_confidence = self.domain_classifier.classify_domain(candidate_text)
                
                if enforce_domain:
                    # Strict separation between IT and healthcare
                    if query_domain == 'it' and candidate_domain == 'healthcare':
                        continue
                    if query_domain == 'healthcare' and candidate_domain == 'it':
                        continue
                ...
                if should_filter:
                    print(f"🚫 Filtered {result.get('email', 'unknown')} due to domain mismatch ({candidate_domain} vs {query_domain})")
                    continue
            ...
            if not filtered_results:
                print("⚠️ Domain filtering produced zero results; returning original list")
                return results
