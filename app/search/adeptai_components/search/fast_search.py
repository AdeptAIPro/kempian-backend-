...
class OptimizedSearchSystem:
    ...
    def _initialize_index(self):
        ...
        if not self.embedding_service or not self.table:
            logger.warning("⚠️ Dependencies not set, cannot build index")
            return

        start_time = time.time()
        logger.info("🏗️ Building FAISS search index...")
        ...
        try:
            ...
            logger.info(f"📊 Processing {len(items)} candidates...")
            ...
            if not valid_candidates:
                logger.warning("⚠️ No valid candidates after processing")
                return
