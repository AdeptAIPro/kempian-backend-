...
This module provides a machine learning-based domain classifier that can
categorize job descriptions and candidate profiles into domains such as
technology, healthcare, finance, education, and marketing.

Classes:
    MLDomainClassifier: Main classifier class with ML and fallback support

Functions:
    get_ml_domain_classifier: Factory function to get or create global instance
...
class MLDomainClassifier:
    ...
    def extract_features(self, text: str) -> np.ndarray:
        ...
        Combines TF-IDF features with optional BERT features if available.
        
        Args:
            text: Input text to extract features from
            
        Returns:
            Feature vector as numpy array. Shape depends on feature configuration:
            - TF-IDF only: (5000,) or vectorizer max_features
            - With BERT: (5000 + 10,) or (max_features + 10,)
        """
        if not text:
            text = ""
    ...
    def train(
        ...
        """
        Train the domain classifier on provided text samples.
        
        This method:
        1. Encodes labels using LabelEncoder
        2. Vectorizes texts using TF-IDF
        3. Splits data into train/test sets
        4. Trains RandomForest classifier
        5. Evaluates and logs accuracy metrics
        
        Args:
            texts: List of text samples for training
            labels: List of domain labels corresponding to texts.
                   Valid labels: 'technology', 'healthcare', 'finance', 
                   'education', 'marketing', 'unknown'
            test_size: Fraction of data to use for testing (0.0 to 1.0).
                      If 0, no test split is created.
            validation_split: Optional validation split (currently unused,
        ...
        Note:
            Requires at least 10 samples for meaningful training.
            Model is automatically marked as trained after successful training.
        """
    ...
    def classify_domain(self, text: str) -> Tuple[str, float]:
        ...
        Args:
            text: Input text to classify (job description, candidate profile, etc.)
            
        Returns:
            Tuple containing:
            - domain (str): One of 'technology', 'healthcare', 'finance', 
                          'education', 'marketing', or 'unknown'
            - confidence (float): Confidence score between 0.0 and 1.0
                                 
        Example:
            >>> classifier = MLDomainClassifier()
            >>> domain, confidence = classifier.classify_domain("Python developer")
    ...
    def should_filter_candidate(
        ...
        This method implements domain-based filtering logic to exclude candidates
        whose domain doesn't match the query domain when confidence is high.
        
        Args:
            candidate_domain: Domain classification of the candidate
            query_domain: Domain classification of the search query
            candidate_confidence: Confidence score for candidate domain (0.0-1.0)
            query_confidence: Confidence score for query domain (0.0-1.0)
            
        Returns:
            True if candidate should be filtered out (domain mismatch with high confidence),
            False otherwise (domains match, low confidence, or unknown domains)
            
        Note:
            - Never filters if either domain is 'unknown'
            - Never filters if domains match
            - Filters only when both confidences are high (>0.8) and domains differ
            - Filters if candidate confidence is very high (>0.9) and domains differ
        """
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        Returns the top 50 most important features (terms) that contribute
        most to domain classification decisions.
        
        Returns:
            Dictionary mapping feature names (terms) to their importance scores.
            Empty dictionary if model is not trained or feature names unavailable.
    ...
    def save_model(self, path: Optional[str] = None) -> None:
        ...
        Saves the RandomForest classifier, TF-IDF vectorizer, label encoder,
        and training status to a pickle file.
        
        Args:
            path: Optional path to save the model. If None, uses self.model_path.
                 Directory will be created if it doesn't exist.
                 
        Note:
            Only saves if model is trained. Logs warning if no trained model exists.
        """
    ...
    def load_model(self, path: Optional[str] = None) -> None:
        ...
        Loads the RandomForest classifier, TF-IDF vectorizer, label encoder,
        and training status from a pickle file.
        
        Args:
            path: Optional path to load the model from. If None, uses self.model_path.
                 
        Note:
            Logs warning if model file doesn't exist. Falls back to rule-based
            classifier if loading fails.
        """
        path = path or self.model_path
...
def get_ml_domain_classifier(
    model_path: Optional[str] = None, 
    use_bert: bool = False
) -> MLDomainClassifier:
    """
    Get or create global ML domain classifier instance (singleton pattern).
    
    This factory function ensures only one instance of the classifier exists,
    which is useful for resource management and consistent state across the application.
    
    Args:
        model_path: Optional path to saved model file. If provided and file exists,
                   model will be loaded automatically.
        use_bert: Whether to enable BERT features (requires transformers library).
                 Defaults to False.
                 
    Returns:
        MLDomainClassifier instance (singleton)
        
    Example:
        >>> classifier = get_ml_domain_classifier()
        >>> domain, conf = classifier.classify_domain("Software engineer")
    """
