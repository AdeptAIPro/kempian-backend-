...
class EnhancedBehavioralPipeline:
    ...
    def analyze_comprehensive_profile(self, source_data: MultiSourceProfile, 
        ...
        try:
            ...
            semantic_score = self.semantic.analyze(all_text, job_description)
            emotions = self.emotions.analyze(all_text)
            behavioral_score = self.scorer.score(all_text, job_description)
            
            # Domain analysis
            domain_type = self.domain_bert.classify_domain(all_text)
            ...
            profile = BehavioralProfile(
                ...
                communication_style=emotions.get('communication_style', 'professional'),
                work_style=emotions.get('work_style', 'collaborative'),
                risk_factors=emotions.get('risk_factors', []),
                strengths=emotions.get('strengths', []),
                recommendations=emotions.get('recommendations', [])
            )
            ...
            return {
                'behavioral_profile': BehavioralProfile(
                    ...
                    recommendations=['Consider detailed review']
                ),
                'career_trajectory': None,
