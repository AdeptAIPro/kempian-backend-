...
def search() -> tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        try:
            ...
            # Ensure data is properly formatted as a dictionary
            if isinstance(results, list):
                # Wrap list results in a dictionary
                response_data = {"results": results}
            elif isinstance(results, dict):
                # Remove pagination from data if it was added there
                response_data = {k: v for k, v in results.items() if k != "pagination"}
            else:
                # Fallback: wrap in a dictionary
                response_data = {"results": results if isinstance(results, list) else []}
