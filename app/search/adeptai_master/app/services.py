...
# Try to import SageMaker client (may not be available)
SageMakerClientManager = None
try:
    ...
    from Sagemaker.sagemaker_client import SageMakerClientManager  # type: ignore
except ImportError:
    SageMakerClientManager = None
...
class ServiceContainer:
    ...
    def initialize_services(self, settings: Settings) -> None:
        ...
        # Initialize SageMaker integrations first (if enabled)
    ...
    def _initialize_sagemaker(self, settings: Settings) -> None:
        """Initialize SageMaker client manager when configured."""
        try:
            if SageMakerClientManager is None:
                self.register('sagemaker', None)
                logger.info("SageMaker module not available - skipping")
                return
            if not getattr(settings, 'use_sagemaker', False):
                self.register('sagemaker', None)
                logger.info("SageMaker disabled by configuration - skipping")
                return
            config = settings.get_sagemaker_config() if hasattr(settings, 'get_sagemaker_config') else {}
            region = getattr(settings, 'sagemaker_region', getattr(settings, 'aws_region', 'ap-south-1'))
            manager = SageMakerClientManager.from_config(config, region=region)
            self.register('sagemaker', manager)
            logger.info("SageMaker client manager initialized")
