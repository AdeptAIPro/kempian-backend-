    ...
    try:
        ...
        if _dependencies_available and 'boto3' in globals():
            ...
            if aws_access_key and aws_secret_key:
                ...
                # Full scan - no limits, get ALL candidates
                print(f"📊 Scanning DynamoDB table: {table_name} (loading ALL candidates)")
                all_items = []
                scan_kwargs = {
                    'Limit': 100,  # Per-request limit (DynamoDB limit, not our limit)
...
class OptimizedDomainClassifier:
    ...
    def __init__(self):
        ...
        try:
            ...
            endpoint = os.getenv('SAGEMAKER_DOMAIN_CLASSIFIER_ENDPOINT')
            ...
            if use_sagemaker and endpoint:
                ...
                self.ml_classifier = SageMakerDomainClassifierClient(endpoint_name=endpoint, region=region)
...
class OptimizedSearchSystem:
    ...
    def __init__(self, background_init=False, use_custom_llm=True):
            ...
            if _use_sm and _ltr_ep:
                ...
                class _LTRAdapter:
                    ...
                    def predict(self, query, candidates, feature_scores=None):
                        ...
                        # Fallback
                        return [0.0] * len(candidates)
                self.ltr_model = _LTRAdapter(_ltr_ep, _region)
        ...
        try:
            _use_sm = os.getenv('USE_SAGEMAKER', 'false').lower() in ('1', 'true', 'yes')
            _ep = os.getenv('SAGEMAKER_LLM_ENHANCER_ENDPOINT')
            _region = os.getenv('SAGEMAKER_REGION', os.getenv('AWS_REGION', 'ap-south-1'))
            if _use_sm and _ep:
                from Sagemaker.sagemaker_client import SageMakerLLMQueryEnhancerClient  # type: ignore
                class _EnhancerAdapter:
                    def __init__(self, endpoint: str, region: str):
                        self.client = SageMakerLLMQueryEnhancerClient(endpoint, region)
                ...
                self.llm_query_enhancer = _EnhancerAdapter(_ep, _region)
        ...
        try:
            _use_sm = os.getenv('USE_SAGEMAKER', 'false').lower() in ('1', 'true', 'yes')
            _ep = os.getenv('SAGEMAKER_JOB_FIT_ENDPOINT')
            _region = os.getenv('SAGEMAKER_REGION', os.getenv('AWS_REGION', 'ap-south-1'))
            if _use_sm and _ep:
        ...
        try:
            ...
            if _use_sm and _ep:
                ...
                self.embedding_service = _EmbeddingServiceAdapter(_ep, _region)
    ...
    def _init_custom_llm_components(self):
        ...
        if self.use_custom_llm:
            ...
            sm_emb_ep = os.getenv('SAGEMAKER_EMBEDDING_ENDPOINT')
                ...
                try:
                    ...
                    class _EmbeddingAdapter:
                        ...
                        def build_index(self, *_args, **_kwargs):
                            self.is_index_ready = True
                    self.custom_embedding_model = _EmbeddingAdapter(sm_emb_ep, sm_region)
    ...
    def search(self, query: str, top_k: int = 10, **kwargs):
        ...
        if enable_domain_filtering and results:
            ...
            results = self._apply_domain_filtering(query, results)
            ...
            for result in results:
                ...
                if 'skills' in result:
                    ...
                    if isinstance(skills_data, list):
                        candidate_skills.extend([str(s) for s in skills_data if s])
                    elif isinstance(skills_data, str):
                        candidate_skills.append(skills_data)
                
                # IMPROVED: Extract from resume text with better skill detection
                if 'resume_text' in result and result['resume_text']:
                    resume_text = result['resume_text'].lower()
                    # Use comprehensive tech skills list (same as query extraction)
                    tech_skills = {
                        'programming': ['python', 'java', 'javascript', 'typescript', 'c++', 'c#', 'go', 'rust', 'ruby', 
                                      'php', 'swift', 'kotlin', 'scala', 'r', 'matlab', 'perl', 'bash', 'shell'],
                        'frameworks': ['react', 'vue', 'angular', 'node.js', 'nodejs', 'django', 'flask', 'fastapi', 
                    ...
                    # IMPROVED: Check for all skills in resume text with better matching
                    for category, skill_list in tech_skills.items():
                        for skill in skill_list:
                            ...
                            if re.search(skill_pattern, resume_text):
                                normalized_skill = self._normalize_skill(skill)
                                if normalized_skill not in candidate_skills:
                                    candidate_skills.append(normalized_skill)
                            # Also check substring match for longer skills (more lenient)
                            elif len(skill_lower) >= 4:
                                # Check if skill appears as substring (but not as part of another word)
                                # Use lookahead/lookbehind to ensure it's not part of a longer word
                ...
                if skill_score > 0:
                    # Candidates with skill matches get good weight on skill match
                    # Use skill_score as primary, but allow more influence from other scores
                    result['final_score'] = 0.7 * skill_score + 0.3 * original_score
                    result['skill_boost'] = skill_score - original_score
    ...
    def _apply_domain_filtering(self, query: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for result in results:
                ...
                # Don't filter if query domain is not specific (general/unknown) - skill matching is priority
                if query_domain in ('general', 'unknown', '') or query_confidence < 0.5:
                    should_filter = False  # Don't filter if query domain is not specific
                else:
