...
class LearningToRankModel:
    ...
    def _compute_skill_overlap(self, query: str, skills: List[str]) -> float:
        """Compute skill overlap score with skill taxonomy if available"""
        if not skills:
            return 0.0
        
        # Try to use skill taxonomy if available
        try:
            from skill_taxonomy import get_skill_taxonomy
            taxonomy = get_skill_taxonomy()
            if taxonomy:
                # Extract required skills from query (simplified)
                query_lower = query.lower()
                required_skills = []
                for skill_name in taxonomy.skills.keys():
                    if skill_name in query_lower or any(alias in query_lower for alias in taxonomy.skills[skill_name].aliases):
                        required_skills.append(skill_name)
                
                if required_skills:
                    # Use taxonomy for matching
                    match_result = taxonomy.calculate_skill_match_score(
