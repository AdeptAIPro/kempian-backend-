...
class InstantSearchEngine:
    ...
    def _load_candidates_from_source(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info("📊 Scanning DynamoDB with optimized settings (loading ALL candidates)...")
            all_items = []
            scan_kwargs = {
    ...
    def search(self, query: str, limit: int = 10, top_k: int = None, 
               include_behavioural_analysis: bool = False, 
               enable_domain_filtering: bool = False, **kwargs) -> List[Dict]:
        """
        Ultra-fast instant search with sub-millisecond response times
        
        Args:
            query: Search query
            limit: Maximum number of results (deprecated, use top_k)
            top_k: Maximum number of results
            include_behavioural_analysis: Whether to include behavioral analysis (not supported in instant search)
            enable_domain_filtering: Whether to enable domain filtering (not supported in instant search)
    ...
    def get_performance_stats(self) -> Dict:
        """Get performance statistics (alias for get_stats for compatibility)"""
