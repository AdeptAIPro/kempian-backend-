...
class CandidateData:
    ...
    def __post_init__(self):
        ...
        # Compress skills list - handle None values
        self.skills = [skill.strip().lower() for skill in self.skills if skill and isinstance(skill, str) and skill.strip()]
        # Normalize name - handle None
        if self.full_name:
        ...
        if isinstance(self.experience, str):
            try:
                self.experience = int(float(self.experience))
            except (ValueError, TypeError):
                self.experience = 0
        elif self.experience is None:
            self.experience = 0
        else:
            try:
...
class OptimizedCandidateCache:
    ...
    def add_candidate(self, candidate_data: Dict) -> bool:
        ...
        try:
            with self._lock:
                ...
                # Get experience and convert to int
                ...
                candidate = CandidateData(
                    ...
                    source_url=candidate_data.get('sourceURL') or '',
    ...
    def _calculate_score(self, candidate: CandidateData, query_words: Set[str]) -> float:
        ...
        seniority_words = {'senior', 'lead', 'principal', 'architect'}
        if any(word in query_words for word in seniority_words):
            # Ensure experience is an integer
