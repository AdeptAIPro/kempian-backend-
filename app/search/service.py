...
def _normalize_candidate_profile(candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    full_name = candidate.get('full_name') or candidate.get('FullName') or candidate.get('name') or ''
    ...
    if not email:
        for key in ('emails', 'emailAddresses', 'email_addresses'):
            ...
            if isinstance(emails_value, list):
                ...
                if email:
                    break
    phone = candidate.get('phone') or candidate.get('Phone') or contact.get('phone') or ''
    ...
    resume_text = candidate.get('resume_text') or candidate.get('resumeText') or ''
...
class FallbackAlgorithm:
    ...
    def keyword_search(self, job_description, top_k=20):
        ...
        # Check if domain filtering will be applied
        ...
        for idx in top_indices:
            ...
            location = normalized['location'] or candidate.get('location', 'Unknown')
            ...
            results.append({
                'email': email_value,
                'full_name': normalized['full_name'] or candidate.get('full_name', 'Unknown'),
                'FullName': normalized['full_name'] or candidate.get('FullName') or candidate.get('name', 'Unknown'),
                'phone': phone_value,
        ...
        logger.info(f"Optimized keyword search completed in {elapsed_time:.2f}s, found {len(results)} candidates")
    ...
    def _extract_skills_from_job(self, job_description):
        ...
        skill_patterns = [
            ...
            r'\b(rn|registered nurse|lpn|licensed practical nurse|cna|certified nursing assistant)\b',
            r'\b(nurse practitioner|patient care|clinical|clinical care|care plan|careplanning|case management)\b',
            r'\b(medication administration|med administration|med pass|phlebotomy|blood draw|injection)\b',
            r'\b(charting|ehr|emr|electronic health record|epic|cerner|meditech)\b',
            r'\b(icu|ccu|er|emergency room|med-surg|medical surgical|long-term care|ltc)\b',
            r'\b(bcls|acls|cpr|first aid|basic life support|advanced cardiac life support)\b',
            r'\b(therapist|occupational therapy|physical therapy|respiratory therapy)\b',
            r'\b(healthcare|medical|hospital|clinic|patient safety|patient advocacy)\b'
        ]
    ...
    def _calculate_domain_alignment(self, candidate: Dict[str, Any], job_description: str) -> float:
        ...
        try:
            ...
            if job_domain_lower == 'healthcare':
                # Penalize clear IT-focused candidates for healthcare roles
                if candidate_domain_lower in {'it/tech', 'technology', 'tech'}:
                    return -10.0
            if job_domain_lower == 'it/tech':
                if candidate_domain_lower in {'healthcare', 'medical'}:
                    return -6.0

            # Mild penalty for other mismatches
            return -4.0
    ...
    def _calculate_skill_match_ratio(self, candidate_skills, job_skills):
        ...
        matches = sum(1 for skill in job_skills_lower if skill in candidate_skills_lower)
...
class AdeptAIMastersAlgorithm:
    ...
    'code blue', 'rapid response', 'falls risk', 'pressure ulcer', 'restraints','cna','word','wound' 'care','charting'
        ...
        for keyword in keywords_lower:
            ...
            for tech_kw in tech_keywords:
                if tech_kw in keyword or keyword in tech_kw:
                    tech_count += 1
                    break
            
            # Check for healthcare keywords
            for health_kw in healthcare_keywords:
                if health_kw in keyword or keyword in health_kw:
                    healthcare_count += 1
                    break
        
        # Log the detection results for debugging
        logger.info(f"Domain detection - Tech: {tech_count}, Healthcare: {healthcare_count}")
        
        # Determine domain with improved logic
        if healthcare_count > tech_count and healthcare_count > 0:
            logger.info("Domain classified as Healthcare")
            return "Healthcare"
        elif tech_count > healthcare_count and tech_count > 0:
            logger.info("Domain classified as IT/Tech")
            return "IT/Tech"
        else:
            ...
            try:
                ...
                if original_results:
                    ...
                    for result in original_results:
                        formatted_result = self._format_original_result_for_frontend(result, job_description)
    ...
    def _apply_domain_filter(self, candidates: List[Dict[str, Any]], job_domain: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Domain filter '{job_domain}': {len(filtered)} candidates matched out of {len(candidates)} total")
    ...
    def _format_original_result_for_frontend(self, original_result: Dict[str, Any], job_description: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            email = normalized['email'] or original_result.get('email', 'unknown')
            full_name = normalized['full_name'] or original_result.get('full_name') or original_result.get('FullName', 'Unknown')
            phone = normalized['phone'] or original_result.get('phone', 'Not provided')
            location = normalized['location'] or original_result.get('location', 'Unknown')
    ...
    try:
        ...
        try:
            ...
            if service and service.is_ready():
                logger.info("Using optimized parallel search system for semantic match")
                result = search_optimized(job_description, top_k=20, use_ultra_fast=True)
                ...
                if result.get('results'):
                    try:
                        ...
                        enhanced_results = enhance_search_accuracy(job_description, result['results'], top_k=20)
