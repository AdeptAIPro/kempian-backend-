...
class FallbackAlgorithm:
    ...
    def keyword_search(self, job_description, top_k=20):
        ...
        job_required_skills = self._extract_skills_from_job(job_description)
        ...
        if job_required_skills or job_domain_lower:
            ...
            for idx, candidate in enumerate(filtered_candidates):
                ...
                # Check skill match (require at least 30% match for required skills)
                if job_required_skills:
                    ...
                    matched_skills = [skill for skill in job_skills_lower if any(skill in cs or cs in skill for cs in candidate_skills_lower)]
                    
                    logger.debug(f"[DEBUG] Skill match - Required: {job_skills_lower}, Matched: {matched_skills}, Ratio: {skill_match_ratio:.2%}")
                    
                    # Adaptive threshold: Lower for healthcare jobs, higher for others
                    # Healthcare jobs often have many specific skills, so we accept candidates with at least 10% match
                    # For other domains, require 20% match
                    skill_threshold = 0.10 if job_domain_lower == 'healthcare' else 0.20
                    
                    # Also check if candidate has at least one relevant skill (even if below threshold)
                    has_relevant_skill = len(matched_skills) > 0
                    
                    # REJECT only if skill match is too low AND no relevant skills found
                    if skill_match_ratio < skill_threshold and not has_relevant_skill:
                        rejected_count += 1
                        rejected_skill_mismatch += 1
                        logger.warning(f"[DEBUG] REJECTED {candidate_name}: Insufficient skill match ({skill_match_ratio:.2%}, matched: {matched_skills})")
                        continue
                    
                    logger.info(f"[DEBUG] ACCEPTED {candidate_name}: Skill match {skill_match_ratio:.2%} (matched: {matched_skills})")
        ...
        for idx in top_indices:
            ...
            # FINAL SAFETY CHECK: Verify skill match before including in results
            if job_required_skills:
                candidate_skills = candidate.get('skills', [])
                if isinstance(candidate_skills, str):
                    candidate_skills = [s.strip() for s in candidate_skills.split(',')]
                ...
                # Calculate final skill match
                matches = sum(1 for skill in job_skills_lower if any(skill in cs or cs in skill for cs in candidate_skills_lower))
                final_skill_match = matches / len(job_skills_lower) if job_skills_lower else 0.0
                
                # Skip candidates with zero skill match for required skills
                if final_skill_match == 0.0:
                    candidate_name = candidate.get('full_name') or candidate.get('FullName') or candidate.get('name', 'Unknown')
                    logger.warning(f"[DEBUG] FINAL CHECK: Skipping {candidate_name} - zero skill match (candidate skills: {candidate_skills_lower[:5]}, required: {job_skills_lower[:5]})")
                    continue
                else:
                    matched_skills = [skill for skill in job_skills_lower if any(skill in cs or cs in skill for cs in candidate_skills_lower)]
                    logger.debug(f"[DEBUG] FINAL CHECK: Including candidate with {final_skill_match:.2%} skill match (matched: {matched_skills})")
                    ...
                    for result in original_results:
                        ...
                        if job_required_skills or job_domain_lower:
                            ...
                            if job_required_skills:
                                ...
                                logger.debug(f"[DEBUG] ACCEPTED {candidate_name}: Skill match {skill_match_ratio:.2%}")
                    ...
                    if len(formatted_results) == 0:
                        ...
                        if len(formatted_results) == 0:
                            ...
                            try:
                                ...
                                logger.info(f"[DEBUG] Fallback algorithm found {len(formatted_results)} candidates")
