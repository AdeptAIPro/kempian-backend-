...
try:
    ...
    logger.warning(f"⚠️ Original AdeptAI algorithm not available: {e}")
...
class AdeptAIMastersAlgorithm:
    ...
    def __init__(self):
        ...
        self._faiss_index_path = os.getenv('FAISS_INDEX_PATH', './indexes/resume_index')
        self._faiss_index_loaded = False
        try:
            import faiss
            self._faiss = faiss
        except ImportError:
    ...
    def _load_candidates_from_dynamodb(self):
        """Load candidates from DynamoDB and handle FAISS index persistence"""
        if table:
            try:
                response = table.scan()
                ...
                # If original algorithm is available, index the candidates and persist FAISS index
                if self.enhanced_system and candidates:
                    try:
                        # Try to load FAISS index from disk if available and up-to-date
                        if self._faiss and os.path.exists(self._faiss_index_path):
                            index = self._faiss.read_index(self._faiss_index_path)
                            if hasattr(index, 'ntotal') and index.ntotal == len(candidates):
                                self.enhanced_system.faiss_index = index
                                self.faiss_index = index
                                self._faiss_index_loaded = True
                                logger.info(f"✅ Loaded FAISS index from {self._faiss_index_path} with {index.ntotal} vectors")
                            else:
                                logger.info(f"⚠️ FAISS index on disk is out of sync (index: {getattr(index, 'ntotal', '?')}, candidates: {len(candidates)}), rebuilding...")
                                self.enhanced_system.index_candidates(candidates)
                                if hasattr(self.enhanced_system, 'faiss_index') and self.enhanced_system.faiss_index is not None:
                                    self._faiss.write_index(self.enhanced_system.faiss_index, self._faiss_index_path)
                                    self.faiss_index = self.enhanced_system.faiss_index
                                    logger.info(f"✅ Rebuilt and saved FAISS index to {self._faiss_index_path}")
                        else:
                            self.enhanced_system.index_candidates(candidates)
                            if self._faiss and hasattr(self.enhanced_system, 'faiss_index') and self.enhanced_system.faiss_index is not None:
                                self._faiss.write_index(self.enhanced_system.faiss_index, self._faiss_index_path)
                                self.faiss_index = self.enhanced_system.faiss_index
                                logger.info(f"✅ Built and saved FAISS index to {self._faiss_index_path}")
                            else:
                                logger.info("⚠️ FAISS not available or index not built")
                        logger.info("✅ Candidates indexed in original algorithm")
                    except Exception as e:
                        logger.error(f"❌ Error indexing candidates or handling FAISS index: {e}")
            except Exception as e:
    ...
    def keyword_search(self, job_description, top_k=20):
        """Advanced search using the original AdeptAI algorithm"""
    ...
    def semantic_match(self, job_description, use_gpt4_reranking=True):
        ...
        try:
            ...
            if self.enhanced_system:
                ...
                fallback_results = self.fallback_algorithm.keyword_search(job_description, top_k=20)
                return {'results': fallback_results, 'summary': f"Fallback algorithm results: {len(fallback_results)} candidates found"}
        except Exception as e:
...
def get_algorithm_instance():
    """Get or create the algorithm instance"""
...
def semantic_match(job_description):
    ...
    return algorithm.semantic_match(job_description)
