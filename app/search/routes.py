...
from app.utils.quota_exemption import is_quota_exempt
...
def jd_search():
    try:
        ...
        # Check if user is quota exempt (whitelisted)
        is_exempt = is_quota_exempt(user.email)
        
        # Initialize trial_status
        trial_status = None
        
        if is_exempt:
            logger.info(f"User {user.email} is quota exempt - skipping quota checks")
        else:
    ...
    try:
        ...
        if is_exempt:
            return jsonify({
                ...
                'is_trial': False,
                'is_exempt': True,
                'message': 'Quota exempt - unlimited searches'
            }), 200
