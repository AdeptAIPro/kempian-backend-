...
class ProductionUnlimitedQuotaManager:
    ...
    def remove_unlimited_user(self, email: str, removed_by: str) -> bool:
        """Remove unlimited quota for a user"""
        try:
            email = email.lower()
            ...
            user.removed_date = datetime.utcnow()
            
            db.session.commit()
            logger.info(f"Removed unlimited quota for user {email}")
            return True
    ...
    def list_unlimited_users(self) -> List[Dict[str, Any]]:
        """List all unlimited quota users"""
        try:
            users = UnlimitedQuotaUser.query.all()
