...
def stafferlink_status():
    ...
    integration = StafferlinkIntegration.query.filter_by(user_id=user.id).first()
    if integration:
        logger.info('Stafferlink integration found.')
        return jsonify({
            'connected': True, 
        ...
        logger.info('Stafferlink integration not found.')
        return jsonify({'connected': False}), 200
...
def stafferlink_disconnect():
    ...
    if integration:
        ...
        db.session.commit()
        logger.info('Stafferlink integration deleted.')
        return jsonify({'message': 'Stafferlink integration disconnected successfully'}), 200
    else:
        logger.info('No Stafferlink integration to delete.')
