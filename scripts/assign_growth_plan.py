...
from app.models import User, Plan, Tenant
...
def assign_growth_plan_to_emails(emails):
    ...
    with app.app_context():
        try:
            # Find Growth Plan - try multiple variations and always use the latest (highest ID)
            growth_plan = Plan.query.filter_by(name="Growth").order_by(Plan.id.desc()).first()
            if not growth_plan:
                # Try "Growth Plan"
                growth_plan = Plan.query.filter_by(name="Growth Plan").order_by(Plan.id.desc()).first()
            if not growth_plan:
                # Try case-insensitive search for "Growth"
                growth_plan = Plan.query.filter(Plan.name.ilike("growth")).order_by(Plan.id.desc()).first()
            if not growth_plan:
                # Try case-insensitive search for "Growth Plan"
